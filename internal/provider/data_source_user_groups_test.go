...
func Test_data_source_user_groups_test(t *testing.T) {
		...
		Steps: []resource.TestStep{
			{
				Config: fmt.Sprintf(`
                    ...
                    data "slack_user_groups" "test" {
                    }

                    output "user_groups" {
                        value = data.slack_user_groups.test
                    }
                `, slackAPIToken),
				...
				Check: resource.TestCheckFunc(func(s *terraform.State) error {
					...
					_, ok := s.RootModule().Resources["data.slack_user_groups.test"]
					if !ok {
						return fmt.Errorf("resource not found: data.slack_user_groups.test")
					}
