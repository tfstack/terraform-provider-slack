...
func Test_data_source_conversations_test(t *testing.T) {
		...
		Steps: []resource.TestStep{
			{
				Config: fmt.Sprintf(`
                    ...
                    data "slack_conversations" "test" {
                    }

                    output "conversations" {
                        value = data.slack_conversations.test
                    }
                `, slackAPIToken),
				...
				Check: resource.TestCheckFunc(func(s *terraform.State) error {
					...
					_, ok := s.RootModule().Resources["data.slack_conversations.test"]
					if !ok {
						return fmt.Errorf("resource not found: data.slack_conversations.test")
					}
