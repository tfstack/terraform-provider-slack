		...
		Steps: []resource.TestStep{
			{
				Config: `
					output "test" {
						value = provider::slack::http_request("http://httpstat.us", "GET", "", {}).status_code
					}
				`,
		...
		Steps: []resource.TestStep{
			{
				Config: `
						output "test" {
							value = provider::slack::http_request("http://httpstat.us/404", "GET", "", {}).status_code
						}
					`,
				Check: resource.ComposeAggregateTestCheckFunc(
		...
		Steps: []resource.TestStep{
			{
				Config: `
					output "test" {
						value = provider::slack::http_request("http://httpstat.us/500", "GET", "", {}).status_code
					}
				`,
				Check: resource.ComposeAggregateTestCheckFunc(
